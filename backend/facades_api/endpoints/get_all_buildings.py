...
from facades_api.dto import geojson

from .routers import buildings_router


@buildings_router.get("/all_buildings", status_code=status.HTTP_200_OK)
async def get_all_buildings_geojson(
    ...
    return await geojson.GeoJSONDto.from_list(buildings, crs=crs_obj)


@buildings_router.get("/screen_buildings", status_code=status.HTTP_200_OK)

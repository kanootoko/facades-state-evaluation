...
from facades_api.db.entities import deffect_types, deffects, marks, photos
...
async def classify_deffects(photo: bytes) -> list[ClassificationResult]:
    """
    Create a request to classification service and get a list of deffects.
    """
    response = requests.post(app_settings.classifier_endpoint, data=photo, timeout=60)
    if response.status_code != 200 or "deffects" not in (response_data := response.json()):
        raise DeffectsClassificationError()
    return [ClassificationResult.from_dict(item) for item in response_data["deffects"]]
...
async def save_classification_results(
    ...
    """
    Add a row to `marks` table and link new `deffects` enities to it. Set user_id=0 as it should be the robot account.
    ...
    statement = select(deffect_types.c.name, deffect_types.c.id)
    d_t = dict((await conn.execute(statement)).fetchall())
    for result in classification_results:
        statement = insert(deffects).values(
            mark_id=mark_id,
            author_id=0,
            x=result.box[0],
            y=result.box[1],

...
class GeoJSONResponse(BaseModel):
    ...
    def from_df(
        ...
        return cls(
            crs=crs,
            features=[Feature.from_series(row, geometry_column, include_nulls) for _, row in data_df.iterrows()],
        )
    ...
    def from_list(
        ...
        """
        if isinstance(next(iter(features), None), Row):
            return cls(
                crs=crs,
                features=[Feature.from_row(feature, geometry_field, include_nulls) for feature in features],
            )
        return cls(
            crs=crs,
            features=[Feature.from_dict(feature, geometry_field, include_nulls) for feature in features],
        )
